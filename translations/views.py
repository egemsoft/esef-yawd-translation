...
class GenerateTranslationMessagesView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if settings.LOCALE_PATHS:
            ...
            self.error = _(
                '<b>Configuration error!</b> Please set the LOCALE_PATHS project setting to allow the creation of a unified messages catalog.')
...
class TranslationMessagesView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['has_change_object_permission'] = self.request.user.has_perm(
            opts.app_label + '.' + opts.get_change_permission(), self.language.pk)

        if not settings.LOCALE_PATHS:
            context['error'] = _('<b>Configuration error!</b> Please set the ' \
                                 'LOCALE_PATHS project setting to allow the ' \
                                 'creation of a unified messages catalog.')
            return context
        ...
        if not os.path.exists(po_path) or not context['lang_files']:
            context['warning'] = _('The system does not appear to have any ' \
                                   'translation messages for this language. ' \
                                   'Please use the "Generate messages" button.')
...
class TranslationMessagesEditView(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(TranslationMessagesEditView, self).dispatch(request, *args,
                                                                 **kwargs)
    ...
    def get_initial(self):
        ...
        if settings.LOCALE_PATHS:
            # check if the folder for this language exists and attempt
            # to create it if id does not exist
            self.po_path = os.path.join(settings.LOCALE_PATHS[0],
                                        to_locale(self.language.name), 'LC_MESSAGES')
        else:
            raise Http404
    ...
    def form_valid(self, form):
        try:
            ...
            domain = 'django.po' if self.po_file.endswith('django.po') \
                else 'djangojs.po'
            ...
            messages.add_message(self.request, messages.SUCCESS,
                                 _(('The file %(file)s was succesfuly updated.' \
                                    % {'file': self.po_file})))
        except:
            messages.add_message(self.request, messages.ERROR,
                                 _(('The file %(file)s could not be saved.' \
                                    % {'file': self.po_file})))
